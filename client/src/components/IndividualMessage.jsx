...
export default function MessageSnapshot({messageDetails, own}) {
  ...
  const handleTextAreaKeyDown = async (e) => {
    if (e.keyCode === 13){
      try {
        await fetch(`https://umbrella-talk-2b5322zw5-ryans-projects-20a8834f.vercel.app/api/conversation/${conversationId}/messages/${messageDetails._id}`, {
          method: 'PUT',
          headers: {
  ...
  const fetchAltUserProfileImage = async () => {
    try {
      ...
      const userList = await fetch("https://umbrella-talk-2b5322zw5-ryans-projects-20a8834f.vercel.app/api/users", {
            method: "GET", 
            headers: {'Content-Type': 'application/json'}
          });
