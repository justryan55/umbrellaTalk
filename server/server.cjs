...
app.put("/api/users/:id", async (req, res, next) => {
  ...
  try {
    const user = await User.findByIdAndUpdate(
      userId,
      { name: updateUsername, profilePictureID: updateProfileImage },
      { new: true }
    );
    res.send(user);
  } catch (err) {
...
app.get("/api/:userId/conversation", async (req, res, next) => {
  try {
    ...
    const conversationWithLatestMessage = await existingConversations.map(
      (conversation) => {
        const latestMessage = latestMessages.find(
          (message) => message._id === conversation.id
        );
...
app.delete(
  ...
  async (req, res, next) => {
    try {
      const { conversationId } = req.params;
      const deleteAllMessages = await Conversation.findByIdAndDelete(
        conversationId
      );
...
app.put(
  "/api/conversation/:conversationId/messages/:messageId",
  async (req, res, next) => {
    try {
